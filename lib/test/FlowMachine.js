...
test("resetting a flow machine leaves it in an empty state", function(assert) {
    var flowMachine = newTestFlowMachine(["continue", "reset"]);
    assert.equal(flowMachine.getCurrentState(), undefined);
    assert.equal(flowMachine.isInFlow(), false);
});
...
asyncTest("flow machine calls the callback on each stat change", function(assert) {
    expect(10);
    ...
    flowMachine.onStateChange = function() {
        var e = expected.shift();

        assert.equal(flowMachine.getCurrentState(), e[0]);
        assert.equal(flowMachine.isInFlow(), e[1]);
    ...
    flowMachine.goto("work");
    flowMachine.continue();
    flowMachine.goto("short");
    flowMachine.reset();
    flowMachine.continue();
});
